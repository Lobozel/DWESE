...
<?php
    ...
    function error($mes){
        $_SESSION['error']=$mes;
        header('Location:emodulo.php?id='.$_GET['id']);
        die();
    }
    function editarModulo($n,$h){
        ...
        $_SESSION['mensaje']='Módulo editado con éxito';
        header('Location:modulos.php');
        die();
    }
?>
    ...
    ?>
        <form name="as" action='<?php echo $_SERVER['PHP_SELF']."?id={$_GET['id']}"; ?>' method='POST'>
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="nom" value="<?php echo $miModulo->nomMod; ?>" name='nombre'
                    required>
            </div>
            <div class="form-group">
                <label for="horas">Horas Semanales</label>
                <input type="number" class="form-control" id="horas" value="<?php echo $miModulo->horasSem; ?>" max='8'
                    min='1' name='horas' required>
            </div>
